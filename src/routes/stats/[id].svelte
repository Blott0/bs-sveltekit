<script context="module">
    ...
    export async function load({ page, fetch, session, stuff }) {
        ...
        const user = { uid: session.uid, username: session.username }
        ...
        const friendslist = await res2.json()
...
<script>
	...
	import Playeditor from '$lib/Playeditor.svelte'
...
<section>
    ...
    <div class="result">
        ...
        {#if user.uid === play.addedby || "61c3ae6e68c7da2dd4830719"}
            <label for="openeditor">edit play</label>
            <input hidden id="openeditor" type="checkbox" bind:checked={toggleeditor}>
            {#if toggleeditor}
                <Playeditor {friendslist} userinfo={user} {play} />
            {/if}
        {/if}
        <ul class="players">
